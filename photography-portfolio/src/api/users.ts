...
const BACKEND_ORIGIN = BACKEND_BASE_URL.endsWith('/')
  ? BACKEND_BASE_URL.slice(0, -1)
  : BACKEND_BASE_URL;
const HAS_BACKEND = Boolean(BACKEND_ORIGIN);

const backendRequest = async <T>(path: string, options: RequestInit = {}): Promise<T> => {
  if (!HAS_BACKEND) {
